...
void setup() {
  ...
  */
  ...
  delay(150);  // let ADC/current sensor settle
...
void TX_CAN() {
  ...
  // }
  // digitalWrite(CTX3, LOW);
  bool message_sent = can.write(BMS_data);
  Serial.print("BMS_Car CAN Message: ");
  for (int i = 0; i < BMS_data.len; i++) {
    Serial.print(BMS_data.buf[i], HEX);
    Serial.print(" ");
  }
  digitalWrite(CTX3, LOW);

  if (!message_sent) {
    Serial.println("\nFailed to send BMS_Car CAN message");
  }
  // else{
  //   Serial.println("\nBMS_Car CAN message sent");
  // }
}
